...
// Add remaining V2 functions with temporary names
export const getAdminStatsV2 = onCall(
  async (request: CallableRequest<GetAdminStatsData>) => {
    // Check if user is authenticated and is an admin
    if (!request.auth) {
      return { success: false, message: 'Authentication required' };
...
export const generateOnlineOrderCodesV2 = onCall(
  async (request: CallableRequest<GenerateCodesData>) => {
    // Check if user is authenticated and is an admin
    if (!request.auth) {
      return { success: false, message: 'Authentication required' };
...
export const validateQRCheckInV2 = onCall(
  async (request: CallableRequest<QRCheckInData>) => {
    // Check if user is authenticated
    if (!request.auth) {
      return { success: false, message: 'Authentication required' };
...
export const addAdminRoleV2 = onCall(
  async (request: CallableRequest<AddAdminRoleData>) => {
    // Check if the request is made by an admin
    if (!request.auth) {
      throw new HttpsError(
